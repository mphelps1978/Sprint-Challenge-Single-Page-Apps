...
> Answer: Client side routing is taking pages and data and rendering them from the client, without having to make network requests. It allows for faster page loads, and less network strain.
...
> I had some issues with my internet during this Sprint. While it was up, I created a separate node/expressAPI locally and referred to it instead of the
> server hosted one. Since my app is hooked into a local server environment, you'll need to run _node server.js_ from the _`root`_ directory, and then
> run the client (_yarn/npm start_) from the _`src`_ directory
